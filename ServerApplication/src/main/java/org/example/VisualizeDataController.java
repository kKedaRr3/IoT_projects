...
//    private long firstTimestamp = -1; // Zmienna przechowująca timestamp pierwszego punktu
    ...
    public void updateChart(String data) {
        try {
            ...
            // Jeżeli to pierwszy punkt, ustawiamy firstTimestamp
            if (DataStorage.getFirstTimestamp() == -1) {
                DataStorage.setFirstTimestamp(timestamp);
            }

            // Obliczenie różnicy czasu od pierwszego timestampu
            long timeDifference = timestamp - DataStorage.getFirstTimestamp();

            // Obliczenie przyspieszenia
            double acceleration = Math.sqrt(x * x + y * y + z * z);

            // Tworzenie punktu danych (czas oparty na różnicy)
            XYChart.Data<Number, Number> dataPoint = new XYChart.Data<>(timeDifference, acceleration);
    ...
    private void adjustXAxis() {
        ...
        if (!accelerometerSeries.getData().isEmpty()) {
            ...
            // Ustawianie zakresu osi X na podstawie aktualnych danych
